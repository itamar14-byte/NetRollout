...
import argparse
from Helper import log,test_tcp_port,validate_file_extension
...
def parse_files(device_path, commands_path,verbose=False):
    if validate_file_extension(commands_path,"txt") and validate_file_extension(device_path,"csv"):
        try:
            ...
            log("Devices file successfully processed." + str(len(devices)) + "devices found")
            log("Devices file successfully processed." + str(len(commands)) + "commands will be executed")
...
def push_config(devices, commands, verbose=False):
    ...
    for device in devices:
        if test_tcp_port(device["ip"], device["port"]):
            try:
...
def main():
    ...
    if args.verify is True or args.verify is False:
    ...
    try:
        ...
        if devices and commands:
            ...
            if verify_rollout:
                ...
                log(str(count) + "devices successfully configured")
                log(str(len(devices) - count) + "devices with problems in configuration")
