    ...
    with get_session() as db_session:
        ...
        if user and user.auth_type == "local":
            ...
            if not ldap_server or not ldap_server.is_active:
                ...
                return redirect(url_for("home"))
            if user_bind(ldap_server, username, password):
                if user.is_approved and user.is_active:
                    db_session.expunge(user)
                    login_user(user)
                    redis_client.set(f"user_session:{user.id}", redis_sid(),
                                     ex=86400)
                    audit("auth.login", success=True, username=user.username,
                          actor_id=user.id, detail={"auth_type": "ldap"})
                    return redirect(url_for("dashboard"))
                elif not user.is_approved:
                    flash("User still pending admin approval", "danger")
                    audit("auth.login", success=False, username=username,
                          actor_id=user.id, detail={"reason": "pending_approval"})
                else:
                    flash("User disabled, please check with administrator", "danger")
                    audit("auth.login", success=False, username=username,
                          actor_id=user.id, detail={"reason": "account_disabled"})
                return redirect(url_for("home"))
            else:
                flash("Invalid credentials", "danger")
                audit("auth.login", success=False, username=username,
                      actor_id=user.id, detail={"reason": "ldap_bind_failed"})
                return redirect(url_for("home"))

        else:
            ldap_server = db_session.query(LDAPServer).filter_by(
            ...
            if ldap_server:
                ...
                if groups:
                    ...
                    if match:
                        ...
                        redis_client.set(f"user_session:{user.id}",
                                         redis_sid(), ex=86400)
                        audit("auth.login", success=True, username=username,
                              actor_id=user.id,
                              detail={"auth_type": "ldap", "auto_created":
                                  True, "group_dn": group_dn})
                        return redirect(url_for("dashboard"))
...
def admin_sessions():
    ...
    if keys:
        with get_session() as db_session:
            for key in keys:
                user_id_str = key.decode().replace("user_session:", "")
                try:
    ...
    return render_template("live_sessions.html",
                           local_sessions=[s for s in sessions if s["auth_type"] == "local"],
                           ldap_sessions=[s for s in sessions if s["auth_type"] == "ldap"],
...
def admin_sessions_kick(user_id):
    ...
    redis_client.delete(f"redis_session:{sid.decode()}")
    redis_client.delete(f"user_session:{user_id}")
    audit("admin.session_kick", object_type="User", object_id=user_id, success=True)
...
def admin_server_ldap_new():
    ...
    cn_identifier = request.form.get("cn_identifier", "").strip()
    bind_type = request.form.get("bind_type", "").strip()
    bind_dn = request.form.get("bind_dn", "").strip()
    use_ssl = request.form.get("use_ssl", "").strip()
...
def admin_server_ldap_save(server_id):
    ...
    with get_session() as db_session:
        ...
        srv.port = int(port)
        srv.base_dn = base_dn if base_dn else srv.base_dn
        srv.cn_identifier = cn_identifier if cn_identifier else srv.cn_identifier
        srv.bind_type = bind_type if bind_type else srv.bind_type
...
def admin_server_ldap_test_user(server_id):
    ...
    if not username or not password:
        return jsonify({"status": "error", "message": "missing credntials"})
    with get_session() as db_session:
        srv = db_session.query(LDAPServer).filter_by(id=server_id).first()
        if not srv:
            return jsonify({"status": "error", "message": "Server not "
...
def admin_server_ldap_import(server_id):
    ...
    with get_session() as db_session:
        ...
        audit("admin.ldap_import", success=True,
...
def inventory_bulk_assign():
    ...
    if not data:
        logger.notify("Bulk assign failed: invalid request", "red",
                      important=True)
        return jsonify({"status": "error",
                        "message": "Invalid redis_session_request"}), 400
