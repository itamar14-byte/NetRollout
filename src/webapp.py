...
def start_otp_flow(user):
	...
	audit("auth.login", success=True, username=user.username, actor_id=user.id)
...
def new_start_rollout():
	...
	# The frontend sends repeated "device_ids"
	# fields for the chosen inventory rows.
...
def admin_server():
	...
	db_mode = "external" if current_config.get(
		"POSTGRES_EXTERNAL") == "true" else "bundled"
	redis_mode = "external" if current_config.get(
		"REDIS_EXTERNAL") == "true" else "bundled"
...
def dashboard():
	...
	kpi = build_kpi(kpi_results_30d, kpi_label_map)

	# ── Active job (always own) ───────────────────────────────────────────────
	job_id = redis_client.smembers(f"user_jobs:{current_user.id}")
...
def security_quick_create():
	...
	profile_id = build_security_profile(label, username, password,
